...
void main_loop ( const emf::Params& globalParams,
                 const cosection::Params cosecParams,
                 const std::unique_ptr<RGBDReader>& reader,
                 const std::string& exportpath = "",
                 bool exp_vols = false,
                 const std::string& maskpath = "",
                 const bool background = false,
                 const bool show_slices = false ) {
    cosection::CoSection emf ( globalParams, cosecParams );

    if ( exportpath.length() ) {
        emf.setupOutput ( exp_vols );
    }
    if ( maskpath.length() ) {
    ...
    if ( !background ) {
        window = new cv::viz::Viz3d ( "Co-Section: 3D view" );
        if ( background ) {
    ...
    while ( reader->moreFrames() ) {
        ...
        emf.render ( rendered, window, show_slices );
        ...
        if ( !background ) {
            ...
            namedWindow ( "Co-Section", cv::WINDOW_NORMAL );
            resizeWindow ( "Co-Section", frame.getSize() );
            imshow ( "Co-Section", vis );
...
int main ( int argc, char **argv ) {
    po::options_description options ( "Co-Section: Dynamic tracking and Mapping"
                                      " from RGB-D data with optimization of 3D"
                                      " volumes" );
    ...
    ( "show-slices", po::bool_switch(),
      "Whether to show ESDF slices in the 3D visualization." )
    ...
    cosection::Params cosecParams; // configuration parameters for optimization
    if ( result.count ( "configfile" ) ) {
        ...
        ( "CosecParams.keyFrameFreq",
          po::value<int> ( &cosecParams.keyFrameFreq ) )

        ( "CosecParams.OptParams.wsigma",
          po::value<float> ( &cosecParams.optParams.wsigma ) )
        ( "CosecParams.OptParams.sdfalpha",
          po::value<float> ( &cosecParams.optParams.sdfalpha ) )
        ( "CosecParams.OptParams.sdfbetaHull",
          po::value<float> ( &cosecParams.optParams.sdfbetaHull ) )
        ( "CosecParams.OptParams.sdfbetaInter",
          po::value<float> ( &cosecParams.optParams.sdfbetaInter ) )
        ( "CosecParams.OptParams.FJcyclelength",
          po::value<int> ( &cosecParams.optParams.FJcyclelength ) )
        ;
    ...
    if ( readerReady ) {
        ...
        main_loop ( globalParams, cosecParams, reader, expath,
                    result["export-volumes"].as<bool>(), maskpath,
                    result["background"].as<bool>(),
                    result["show-slices"].as<bool>() );
    }
}
